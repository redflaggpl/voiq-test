...
class GDbStarboxLogRoute extends CLogRoute
{
	/**
...
(level, category, logtime, message, detalles, site, bono, customer, ip, type) VALUES
(:level, :category, :logtime, :message, :detalles, :site, :bono, :customer, :ip, :type)
";
		...
		foreach($logs as $log)
		{
			...
			$detalles=$log[0];
			$site=Yii::app()->params['site_alias'];
			$ip=Yii::app()->request->getUserHostAddress();
			$type=0;
			if(strpos($log[0],"#")!==false)
			{
				$params=explode("#",$log[0]);
				$detalles=$params[0];
				$site=$params[1];
				$bono=$params[2];
				$customer=$params[3];
			...
			$command->bindValue(':detalles',$detalles);
			$command->bindValue(':site',$site);
			$command->bindValue(':bono',$bono);
			$command->bindValue(':customer',$customer);
